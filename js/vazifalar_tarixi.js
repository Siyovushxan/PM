...
//         Promise.all([
//             fetch(`${serverUrl}/api/vazifalar/${taskId}`).then(res => {
...
//                 return { id: taskId, name: 'Noma\'lum vazifa', description: 'Izoh mavjud emas' };
//             }),
//             fetch(`${serverUrl}/api/vazifalar-details-right/${taskId}`).then(res => {
...
//                 return { responsible: 'Noma\'lum', start_date: null, end_date: null, status: 'Noma\'lum' };
//             })
//         ])
//             .then(([basicDetails, rightDetails]) => {
//                 console.log('Asosiy ma\'lumotlar:', basicDetails, 'O\'ng tomon ma\'lumotlari:', rightDetails);
//                 taskName.textContent = `Vazifa nomi: ${basicDetails.name || basicDetails.vazifa_nomi || 'Noma\'lum vazifa'}`;
//                 taskId.textContent = `Vazifa raqami: ${basicDetails.id !== null && basicDetails.id !== undefined ? basicDetails.id.toString() : 'Noma\'lum'}`;
//                 taskDescription.textContent = `Vazifa izohi: ${basicDetails.description || basicDetails.izoh || 'Izoh mavjud emas'}`;
...
//                 fetch(`${serverUrl}/api/projects/${projectId}`).then(res => res.json()).then(project => {
...
//                 taskHistory.innerHTML = chatData.length > 0
//                     ? chatData.map(message => {
//                         const isCurrentUser = message.user_task_id.toString() === currentUserId.toString();
//                         console.log(`Xabar: ${message.matn}, Foydalanuvchi: ${message.user_task_id}, Joriy: ${currentUserId}, Chap/O‘ng: ${isCurrentUser ? 'right' : 'left'}`);
//                         return `
...
//                                 ${message.file_paths ? `<p>Fayllar: ${message.file_paths.split(',').map(file => `<a href="${serverUrl}/uploads/${file.trim()}" download="${file.trim()}">${file.trim()}</a>`).join(', ')}</p>` : ''}
//                             </div>
//                         `;
//                     }).join('')
...
//                         if (!response.ok) throw new Error(`HTTP xatolik: ${response.status} - ${response.statusText}`);
...
//                         openModal(currentTaskId); // Xatolik bo‘lsa ham chatni yangilash
//                     });
//             })
//             .catch(error => {
                ...
                taskHistory.innerHTML = chatData.length > 0
                    ? chatData.map(message => {
                        ...
                        return `
                            <div class="chat-message ${isCurrentUser ? 'right' : 'left'}">
                                <p><strong>${message.fish || 'Noma\'lum'}</strong>: ${message.matn || ''} <small>(${formatDateTime(message.vaqt) || 'N/A'})</small></p>
                                ${message.file_paths && message.file_paths.length > 0 ? `<p>Fayllar: ${message.file_paths.map(file => `<a href="${serverUrl}/uploads/${file}" download="${file}">${file}</a>`).join(', ')}</p>` : ''}
                            </div>
                        `;
                    }).join('')
                    : '<p>Chat tarixi mavjud emas.</p>';
    ...
    window.sendMessage = function () {
        ...
        fetch(`${serverUrl}/api/user/${currentUserId}`)
            ...
            .then(userData => {
                ...
                formData.append('matn', text);
                for (let file of files) {
                    formData.append('file_paths', file); // Har bir faylni alohida qo‘shish
                }
                ...
                console.log('Yuborilayotgan ma\'lumotlar:', {
                    ...
                    matn: text,
                    file_count: files.length,
                    files: files.length > 0 ? Array.from(files).map(f => f.name) : null
                });
