...
async function fetchLoyihalarData() {
  try {
    const response = await fetch('http://localhost:5000/api/loyihalar');
    if (!response.ok) {
      throw new Error('Loyihalarni yuklashda xatolik: ' + response.statusText);
    }
    const loyihalar = await response.json();
    ...
    loyihalar.forEach(loyiha => {
      const status = loyiha.status.toLowerCase();
    ...
    // Umumiy loyihalar sonini hisoblash
    const total = loyihalar.length;
...
async function initLoyihalarDoughnutChart() {
  const { total, doughnutData } = await fetchLoyihalarData();
  ...
  const loyihalarCtx = document.getElementById('loyihaStatistikaGrafik').getContext('2d');
  ...
  // Umumiy loyihalar sonini yuqoridagi divga yozish
  const totalElement = document.querySelector('.loyiha-statistika .total-value');
  if (totalElement) {
  ...
  function simulateHover(index) {
    if (currentIndex >= 0 && currentIndex < filteredData.length) {
      if (loyihalarChart && loyihalarChart.data.datasets[0]) {
        loyihalarChart.data.datasets[0].offset = loyihalarChart.data.datasets[0].offset || new Array(loyihalarChart.data.datasets[0].data.length).fill(0);
        loyihalarChart.data.datasets[0].offset[currentIndex] = 0;
    ...
    if (currentIndex >= 0 && currentIndex < filteredData.length) {
      if (loyihalarChart && loyihalarChart.data.datasets[0]) {
        loyihalarChart.data.datasets[0].offset = loyihalarChart.data.datasets[0].offset || new Array(loyihalarChart.data.datasets[0].data.length).fill(0);
        loyihalarChart.data.datasets[0].offset[currentIndex] = 30;
        loyihalarChart.update();
      }
    }
  }
