...
//                             ${tasks
//                                 .map(task => `
//                                     <tr class="task-row ${task.vazifa_status === 'Yakunlandi' ? (task.vazifa_status_changed_via_edit ? 'edited-completed' : 'completed') : ''}" data-id="${task.id || ''}">
//                                         <td>${index++}</td>
...
//                     if (taskData.vazifa_status === 'Yakunlandi') {
//                         taskRow.classList.remove('completed'); // Ijobiy xulosa rangini olib tashlash
//                         taskRow.classList.add('edited-completed'); // Tahrirlash rangini qo‘shish
//                     } else {
//                         taskRow.classList.remove('completed', 'edited-completed'); // Boshqa statuslarda ranglarni olib tashlash
//                     }
//                 }
//                 editTaskModal.style.display = 'none';
//                 loadTasks(); // Sahifani yangilash
                        ...
                        <tbody>
                            ${tasks
                                .map(task => {
                                    ...
                                    switch (task.vazifa_status) {
                                        case 'Yakunlandi':
                                            className += ' completed'; // Ijobiy xulosa
                                            break;
                                    ...
                                    return `
                                        <tr class="${className}" data-id="${task.id || ''}">
                                            <td>${index++}</td>
                                            <td class="task-name" data-task-id="${task.id}">${task.vazifa_nomi || 'N/A'}</td>
                                            <td>${task.izoh || 'N/A'}</td>
                                            <td>${formatDate(task.vazifa_boshlanish_sanasi) || 'N/A'}</td>
        ...
        try {
            ...
            if (result && result.message === 'Vazifa muvaffaqiyatli yangilandi') {
                ...
                if (taskRow) {
                    const statusCell = taskRow.querySelector('td:nth-child(6)');
                    statusCell.textContent = taskData.vazifa_status;
                    taskRow.classList.remove('planned', 'ongoing', 'completed', 'edited-completed', 'paused');
                    switch (taskData.vazifa_status) {
                        case 'yakunlandi':
                            taskRow.classList.add('edited-completed');
                            break;
                        case 'rejalashtirilmoqda':
                            taskRow.classList.add('planned');
                            break;
                        case 'bajarilmoqda':
                            taskRow.classList.add('ongoing');
                            break;
                        case 'to\'xtatildi':
                            taskRow.classList.add('paused');
                            break;
                    }
                }
                editTaskModal.style.display = 'none';
