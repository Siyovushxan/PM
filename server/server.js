...
//     const sql = 'SELECT * FROM positions ORDER BY name ASC';
...
//     const { name } = req.body;
//     if (!name) {
//         return res.status(400).json({ message: 'Lavozim nomi majburiy!' });
//     }
...
//                 return res.status(400).json({ message: 'Bu lavozim allaqachon mavjud!' });
//             }
//             return res.status(500).json({ message: 'Lavozim qo‘shishda xatolik yuz berdi' });
//         }
...
//     db.query(sql, [name, positionId], (err, result) => {
//         if (err) {
//             console.error('Lavozimni yangilashda xatolik:', err);
//             if (err.code === 'ER_DUP_ENTRY') {
//                 return res.status(400).json({ message: 'Bu lavozim allaqachon mavjud!' });
//             }
//             return res.status(500).json({ message: 'Lavozimni yangilashda xatolik' });
//         }
...
app.post('/api/positions', (req, res) => {
    ...
    if (!name_uz || !name_ru || !name_en) {
        return res.status(400).json({ message: 'Barcha tillardagi lavozim nomlari majburiy!' });
    }
    ...
    if (parent_position_id) {
        ...
        db.query(checkSql, [parent_position_id], (err, results) => {
            if (err || results.length === 0) {
                return res.status(400).json({ message: 'Tanlangan yuqori turuvchi lavozim mavjud emas!' });
            }

            const sql = 'INSERT INTO positions (name_uz, name_ru, name_en, parent_position_id) VALUES (?, ?, ?, ?)';
            db.query(sql, [name_uz, name_ru, name_en, parent_position_id], (err, result) => {
                if (err) {
                    console.error('Lavozim qo‘shishda xatolik:', err);
                    if (err.code === 'ER_DUP_ENTRY') {
                        return res.status(400).json({ message: 'Bu lavozim allaqachon mavjud!' });
...
app.put('/api/positions/:id', (req, res) => {
    ...
    if (parent_position_id && parseInt(parent_position_id) === parseInt(positionId)) {
        return res.status(400).json({ message: 'Lavozim o‘ziga o‘zi yuqori turuvchi lavozim bo‘la olmaydi!' });
    }
    ...
    if (parent_position_id) {
        ...
        db.query(checkSql, [parent_position_id], (err, results) => {
            ...
            const sql = 'UPDATE positions SET name_uz = ?, name_ru = ?, name_en = ?, parent_position_id = ? WHERE id = ?';
            db.query(sql, [name_uz, name_ru, name_en, parent_position_id, positionId], (err, result) => {
                if (err) {
                    console.error('Lavozimni yangilashda xatolik:', err);
                    if (err.code === 'ER_DUP_ENTRY') {
                        return res.status(400).json({ message: 'Bu lavozim allaqachon mavjud!' });
        ...
        const sql = 'UPDATE positions SET name_uz = ?, name_ru = ?, name_en = ?, parent_position_id = ? WHERE id = ?';
        db.query(sql, [name_uz, name_ru, name_en, null, positionId], (err, result) => {
            if (err) {
                console.error('Lavozimni yangilashda xatolik:', err);
                if (err.code === 'ER_DUP_ENTRY') {
                    return res.status(400).json({ message: 'Bu lavozim allaqachon mavjud!' });
...
app.post('/api/departments', (req, res) => {
    ...
    if (!name_uz || !name_ru || !name_en) {
        return res.status(400).json({ message: 'Barcha tillardagi bo‘lim nomlari majburiy!' });
    }
    ...
    if (parent_department_id) {
        ...
        db.query(checkSql, [parent_department_id], (err, results) => {
            if (err || results.length === 0) {
                return res.status(400).json({ message: 'Tanlangan yuqori turuvchi bo‘lim mavjud emas!' });
            }

            const sql = 'INSERT INTO departments (name_uz, name_ru, name_en, parent_department_id) VALUES (?, ?, ?, ?)';
            db.query(sql, [name_uz, name_ru, name_en, parent_department_id], (err, result) => {
                if (err) {
                    console.error('Bo‘lim qo‘shishda xatolik:', err);
                    if (err.code === 'ER_DUP_ENTRY') {
                        return res.status(400).json({ message: 'Bu bo‘lim allaqachon mavjud!' });
                    }
                    return res.status(500).json({ message: 'Bo‘lim qo‘shishda xatolik yuz berdi' });
                }
...
app.put('/api/departments/:id', (req, res) => {
    ...
    if (parent_department_id && parseInt(parent_department_id) === parseInt(departmentId)) {
        return res.status(400).json({ message: 'Bo‘lim o‘ziga o‘zi yuqori turuvchi bo‘lim bo‘la olmaydi!' });
    }
    ...
    if (parent_department_id) {
        ...
        db.query(checkSql, [parent_department_id], (err, results) => {
            ...
            const sql = 'UPDATE departments SET name_uz = ?, name_ru = ?, name_en = ?, parent_department_id = ? WHERE id = ?';
            db.query(sql, [name_uz, name_ru, name_en, parent_department_id, departmentId], (err, result) => {
                if (err) {
                    console.error('Bo‘limni yangilashda xatolik:', err);
                    if (err.code === 'ER_DUP_ENTRY') {
                        return res.status(400).json({ message: 'Bu bo‘lim allaqachon mavjud!' });
                    }
                    return res.status(500).json({ message: 'Bo‘limni yangilashda xatolik' });
                }
                if (result.affectedRows === 0) {
        ...
        const sql = 'UPDATE departments SET name_uz = ?, name_ru = ?, name_en = ?, parent_department_id = ? WHERE id = ?';
        db.query(sql, [name_uz, name_ru, name_en, null, departmentId], (err, result) => {
            if (err) {
                console.error('Bo‘limni yangilashda xatolik:', err);
                if (err.code === 'ER_DUP_ENTRY') {
                    return res.status(400).json({ message: 'Bu bo‘lim allaqachon mavjud!' });
