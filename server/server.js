...
//     db.query(sql, [vazifa_nomi, izoh, vazifa_boshlanish_sanasi, vazifa_tugash_sanasi, vazifa_status, vazifa_masul_hodimi, taskId], (err, result) => {
//         if (err) {
//             console.error('Xatolik:', err);
...
//                 req.session.userId = user.id.toString(); // Sessionga userId sifatida string saqlash
//                 res.json({
...
//     const taskId = req.params.taskId;
//     const sql = 'SELECT ch.user_task_id, ch.task_id, u.FISH AS fish, ch.matn, ch.vaqt, ch.file_paths FROM chat_history ch LEFT JOIN users u ON ch.user_task_id = u.id WHERE ch.task_id = ? ORDER BY ch.vaqt ASC';
//     db.query(sql, [taskId], (err, results) => {
//         if (err) {
...
//     // Fayl yuklashni tekshirish va to'g'ri qiymat tayyorlash
//     if (req.files && req.files.length > 0) {
//         filePaths = req.files.map(file => file.filename).join(',');
//         console.log('Yuklangan fayllar:', req.files.map(file => file.originalname));
//     } else {
//         console.log('Fayl yuklanmadi');
//         filePaths = ''; // Bo'sh string yoki null o'rniga mos qiymat
//     }

//     console.log('Kiritilgan ma\'lumotlar:', { task_id, user_task_id, fish, matn, filePaths }); // Debugging

//     if (!task_id || !user_task_id || !fish || !matn) {
//         return res.status(400).json({ message: 'Barcha majburiy maydonlarni to‘ldiring!' });
//     }

//     // task_id ni son sifatida tekshirish
...
//     // user_task_id ni string sifatida tekshirish
//     const parsedUserTaskId = user_task_id.toString();
//     if (!parsedUserTaskId) {
//         console.error('user_task_id noto‘g‘ri formatda:', user_task_id);
//         return res.status(400).json({ message: 'user_task_id bo‘sh bo‘lmasligi kerak!' });
//     }

//     // Vazifa mavjudligini tekshirish
//     const checkTaskSql = 'SELECT id FROM vazifalar WHERE id = ?';
//     db.query(checkTaskSql, [parsedTaskId], (err, results) => {
//         if (err) {
...
//         db.query(checkUserSql, [parsedUserTaskId], (err, userResults) => {
//             if (err) {
...
//             const sql = 'INSERT INTO chat_history (task_id, user_task_id, fish, matn, vaqt, file_paths) VALUES (?, ?, ?, ?, ?, ?)';
//             db.query(sql, [parsedTaskId, parsedUserTaskId, fish, matn, vaqt, filePaths || null], (err, result) => {
//                 if (err) {
//                     console.error('Xabar saqlashda xatolik:', err.sqlMessage || err.message); // SQL xatosini aniqlash
...
const storage = multer.diskStorage({
    ...
    filename: (req, file, cb) => {
        const fileName = Date.now() + '-' + file.originalname.replace(/[^a-zA-Z0-9.]/g, '_'); // Maxsus belgilarni almashtirish
        console.log('Saqlanayotgan fayl nomi:', fileName);
        cb(null, fileName);
    },
});
const upload = multer({ storage: storage, limits: { fileSize: 10 * 1024 * 1024 } }); // 10MB chegarasi
    ...
    db.query(sql, [taskId], (err, results) => {
        ...
        results.forEach(result => {
            ...
            if (result.file_paths) {
                result.file_paths = result.file_paths.split(',').map(file => file.trim());
            }
        });
        res.json(results || []);
    });
...
app.post('/api/chat-history', upload.array('file_paths', 5), (req, res) => {
    ...
    if (req.files && req.files.length > 0) {
        ...
        console.log('Fayl yuklanmadi');
        filePaths = null; // Bo'sh holatda null qiymat
    }
    ...
    db.query(checkTaskSql, [parsedTaskId], (err, results) => {
        ...
        db.query(checkUserSql, [parsedUserTaskId], (err, userResults) => {
            ...
            const sql = 'INSERT INTO chat_history (task_id, user_task_id, fish, matn, vaqt, file_paths) VALUES (?, ?, ?, ?, ?, ?)';
            db.query(sql, [parsedTaskId, parsedUserTaskId, fish, matn, vaqt, filePaths], (err, result) => {
                if (err) {
                    console.error('Xabar saqlashda xatolik:', err.sqlMessage || err.message); // SQL xatosini aniqlash
                    return res.status(500).json({ message: 'Xabar saqlanmadi', error: err.sqlMessage || err.message });
                }
